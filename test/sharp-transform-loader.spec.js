...
describe('sharp-transform-loader spec', () => {
  ...
  test('sources provided sizes values with webp ext', () => {
    ...
    return runTest(compiler, (window) => {
      expect(Object.keys(window.img.sources)).toEqual(['400w', '400w.webp', '800w', '800w.webp']);
      expect(window.img.srcSet).toContain('400w');
  ...
  test('sources with provided density', () => {
    const compiler = makeCompiler({
      files: {
        'main.js': `window.img = require('${ICELAND}?sizes=1x+1x.webp+2x+2x.webp');`,
      },
      rule: RULE
    });

    return runTest(compiler, (window) => {
      expect(Object.keys(window.img.sources)).toEqual(['1x', '1x.webp', '2x', '2x.webp']);
